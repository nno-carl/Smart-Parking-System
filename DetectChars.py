...
classifier1 = OneVsRestClassifier(KNeighborsClassifier(n_neighbors=1))
model1 = KNeighborsClassifier(n_neighbors=1)
classifier2 = OneVsRestClassifier(RandomForestClassifier(n_estimators = 100, max_depth = 7))
classifier3 = OneVsRestClassifier(LinearSVC(dual=False))
classifier4 = OneVsRestClassifier(MLPClassifier(activation = 'relu',random_state = 1, tol = 0))
...
def TrainModel():
    ...
    try:
        Classifications = np.loadtxt("classifications.txt", np.float32)                  # read in training classifications(labels)
    ...
    try:
        ...
        x = np.array(FlattenedImages)                 
    except:                                                                              #read in training Images information
        print("error, unable to open flattened_images.txt, exiting program\n")  
        os.system("pause")
        return False                                                                     

    Classifications = Classifications.reshape((Classifications.size, 1))      
    yy = label_binarize(Classifications,y)
    print(yy)
    n_class = len(Classifications)
    train, test, train_type, test_type = train_test_split(FlattenedImages, yy, test_size=0.3,random_state = 0)
    
#    model1.fit(train, train_type)
#    predictions = model1.predict(test)
#    acc = model1.score(test, test_type)
#    print("!!!!!!!!Accuracy:{:.2f}%".format(acc * 100))
#    for i in range(test_type[0]):
#        print(accuracy_score(test_type, predictions))
#    
   
    classifier1.fit(train, train_type)
    predictions1 = classifier1.predict(test)
    print(accuracy_score(test_type, predictions1))
    y_score = classifier1.predict_proba(test)
    print(test_type.shape)
    print(y_score.shape)
    precision = dict()
    recall = dict()
    average_precision = dict()
    for i in range(n_class):
        precision[i], recall[i], _ = precision_recall_curve(test_type[:, i],
                                                        y_score[:, i])
    average_precision[i] = average_precision_score(test_type[:, i], y_score[:, i])
    precision["micro"], recall["micro"], _ = precision_recall_curve(test_type.ravel(),
    y_score.ravel())
    average_precision["micro"] = average_precision_score(test_type, y_score,
                                                     average="micro")
    print('Average precision score, micro-averaged over all classes: {0:0.2f}'.format(average_precision["micro"]))
    
    
#    classifier2.fit(train, train_type)
#    predictions2 = classifier2.predict(test)
#    print(predictions2.shape)
#    print(accuracy_score(test_type, predictions2))
#    y_score = classifier2.predict_proba(test)
#    print(test_type.shape)
#    print(y_score.shape)
#    precision = dict()
#    recall = dict()
...
#    classifier3.fit(train, train_type)
#    predictions3 = classifier3.predict(test)
#    print(predictions3)
#    print(accuracy_score(test_type, predictions3))
#    y_score = classifier3.decision_function(test)
#    print(test_type.shape)
#    print(y_score.shape)
#    
#    precision = dict()
...
#    classifier4.fit(train, train_type)
#    predictions4 = classifier4.predict(test)
#    print(accuracy_score(test_type, predictions4))
#    y_score = classifier4.predict_proba(test)
#    print(test_type.shape)
#    print(y_score.shape)
#    precision = dict()
#    recall = dict()
...
#    y_score = model4.predict_proba(test)
#    fpr, tpr, threshold = roc_curve(test_type,y_score[:,-1])
#    roc_auc = auc(fpr,tpr)
#    print("ROC:")
#    print(roc_auc)
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:        
        ...
        possiblePlate.imgThresh = cv2.resize(possiblePlate.imgThresh, (0, 0), fx = 1.4, fy = 1.6)
        ...
        #用knn识别车牌（class possiblePlate全局变量
...
def findListOfListsOfMatchingChars(listOfPossibleChars):#调用1:把整张图里识别到的单个单个char分组（还未match）
                                                        #调用2:把可能车牌里单个单个char分组（即将match）
    ...
    for possibleChar in listOfPossibleChars:                       
        ...
        if len(listOfMatchingChars) < MIN_NUMBER_OF_MATCHING_CHARS:     #互相比较singlepossiblechars里的每个char，如果不一样就计算距离看是不是一组
            continue                            
                                                                                               
        listOfListsOfMatchingChars.append(listOfMatchingChars)     #分组
...
def findListOfMatchingChars(possibleChar, listOfChars):#识别单个单个char是不是一组（通过距离和角度）
...
def removeInnerOverlappingChars(listOfMatchingChars):#对于完全一样或基本一样的char，取小的
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    for currentChar in listOfMatchingChars:                                         # for each char in plate
        ...
        npaResults = classifier4.predict(npaROIResized)
        
        print(npaResults)
        
        strCurrentChar = str(chr(int(npaResults[0])))            # get character from results
