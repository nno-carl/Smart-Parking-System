...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates: # every possible license plate        
...
def findListOfListsOfMatchingChars(listOfPossibleChars):
    ...
    for possibleChar in listOfPossibleChars:                        # for each possible char in the one big list of chars
        ...
        if len(listOfMatchingChars) < MIN_NUMBER_OF_MATCHING_CHARS:     #互相比较singlepossiblechars里的每个char，如果不一样就计算距离看是不是一组if current possible list of matching chars is not long enough to constitute a possible plate
